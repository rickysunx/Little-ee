...
if(is_string_not_empty($keyword) && is_string_not_empty($field)>0) {
	if($field=="orderId") {
		...
		$where_sql = " date_format(o.order_time,'%y-%m-%d')  = ? ";
	}  else if($field=="predictedTime") {
		$where_sql = "  date_format(o.predicted_time,'%y-%m-%d') = ? ";
	}
}
...
if(is_string_not_empty($keyword_01) && is_string_not_empty($field_01)>0) {
	if($field_01=="orderId_01") {
		...
		$where_sql_01 = " date_format(o.order_time,'%y-%m-%d')  = ? ";
	}  else if($field_01=="predictedTime_01") {
		$where_sql_01 = "  date_format(o.predicted_time,'%y-%m-%d') = ? ";
	}
}
...
if(is_string_empty($where_sql_01)) {
	...
	$data = db_query($sql, is_string_empty($where_sql)?array($start_date, $end_date, $shop_id):array($keyword, $start_date, $end_date, $shop_id));
} else if (is_string_empty($where_sql)) {
	$sql = "select * ".$sql_raw
			.(is_string_empty($where_sql_01)?" and b.order_status<>0 ":" and b.order_status<>0 and ".$where_sql_01)
			." and (DATE_FORMAT(o.order_time,'%Y-%m-%d') BETWEEN ? AND ? ) "
			." and o.shop_id = ? "
			." order by order_time desc ".$sql_limit;
	$data = db_query($sql, is_string_empty($where_sql_01)?array($start_date, $end_date, $shop_id):array($keyword_01, $start_date, $end_date, $shop_id));
} else {
	$sql = "select * ".$sql_raw
			.(is_string_empty($where_sql)?" and b.order_status<>0 ":" and b.order_status<>0 and ".$where_sql)
			.(is_string_empty($where_sql_01)?" and b.order_status<>0 ":" and b.order_status<>0 and ".$where_sql_01)
			...
			." and o.shop_id = ? "
			." order by order_time desc ".$sql_limit;
	$data = db_query($sql, array($keyword, $keyword_01, $start_date, $end_date, $shop_id));
}
...
<form id="queryForm" method="get" action="balance_bonus_detail_02.php">
...
<input type="button" value="查看所有订单" onclick="window.location.href='balance_bonus_detail_02.php'"/>
</div>
</form>
